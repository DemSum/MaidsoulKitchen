...
public class VResourceLocation{
    ...
    public static ResourceLocation createMod(String extension) {
        return ResourceLocation.fromNamespaceAndPath(IModInfo.MOD_ID, extension);
    }

    public static ResourceLocation createTypeMod(ResourceLocation recipeType) {
        if (recipeType.getNamespace().isEmpty()) {
            return ResourceLocation.fromNamespaceAndPath(IModInfo.MOD_ID, recipeType.getPath());
        } else {
            return ResourceLocation.fromNamespaceAndPath(IModInfo.MOD_ID + "_" + recipeType.getNamespace(), recipeType.getPath());
        }
    }

    public static ResourceLocation createTypeMod(String modId, String type) {
        return createTypeMod(ResourceLocation.fromNamespaceAndPath(modId, type));
    }

    public static ResourceLocation createTypeMod(IMods modId, String type) {
        return createTypeMod(ResourceLocation.fromNamespaceAndPath(modId.modId(), type));
    }
