...
public class MultiClassAnalysisResult {
    ...
    public void addClassResult(ClassAnalysisResult result) {
        ...
        globalLogs.addAll(result.logs.values());
    }
    ...
    // 导出报告到文件
    public Path exportToFile(Map<String, VerifyExistence.ClazzInfo> allClazzInfo, List<ClassAnalysisResult> errorClassResults, BaseClazzCheckManager<?, ?> checkManager) throws IOException {
        File file = getExportFilePath(checkManager).toFile();
        if (!file.getParentFile().exists()) {
            file.getParentFile().mkdir();
        }
        FileWriter fileWriter = new FileWriter(file);
        fileWriter.write(generateReport(allClazzInfo, errorClassResults, checkManager));
        fileWriter.close();
    ...
    // 生成文本格式报告
    private String generateReport(Map<String, VerifyExistence.ClazzInfo> allClazzInfo, List<ClassAnalysisResult> errorClassResults, BaseClazzCheckManager<?, ?> checkManager) {
        ...
        // 错误类统计信息
        report.append("===== ErrorTasks =====\n");
        report.append("Count: ").append(errorClassResults.size()).append("\n");
        for (ClassAnalysisResult result : errorClassResults) {
            report.append("## ").append(result.uid).append("\n");
            List<String> errorClazzs = result.classes.stream().filter(o -> !result.classExistence.getOrDefault(o, true)).toList();
            if (!errorClazzs.isEmpty()) {
                report.append("### ErrorClasses\n");
                errorClazzs.forEach(e -> {
                    report.append("[Error] ").append(result.getLog(e).getMessage()).append("\n");
                });
            }

            List<String> errorMethods = result.methods.stream().filter(o -> !result.methodExistence.getOrDefault(o, true)).toList();
            if (!errorMethods.isEmpty()) {
                report.append("### ErrorMethods\n");
                errorMethods.forEach(e -> {
                    report.append("[Error] ").append(result.getLog(e).getMessage()).append("\n");
                });
            }

            List<String> errorFields = result.fields.stream().filter(o -> !result.fieldExistence.getOrDefault(o, true)).toList();
            if (!errorFields.isEmpty()) {
                report.append("### ErrorFields\n");
                errorFields.forEach(e -> {
                    report.append("[Error] ").append(result.getLog(e).getMessage()).append("\n");
                });
            }

            List<String> errorMixins = result.mixins.stream().filter(o -> !result.mixinExistence.getOrDefault(o, true)).toList();
            if (!errorMixins.isEmpty()) {
                report.append("### ErrorMixins\n");
                errorMixins.forEach(e -> {
                    report.append("[Error] ").append(result.getLog(e).getMessage()).append("\n");
                });
            }
        }
        ...
        for (ClassAnalysisResult result : classResults) {
            ...
            result.logs.values().forEach(log -> report.append(log).append("\n"));
            report.append("\n");
        }
