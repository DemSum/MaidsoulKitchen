...
最近已提交检查点：`5cb0b7f2`（`Align steamer filter UI with cooking pot`）；本文同时
记录本批尚待提交的烹饪期游荡与补路修复。
...
- 将 `enableLookAndRandomWalk` 任务级约定适配到 1.21.1；MSK 烹饪任务自行搜索整个
  工作点区域，因此选中烹饪任务期间关闭 TLM 通用随机游荡，不受河童罗盘半径大小
  影响。
...
- 最近完整测试：48 tests，0 failures，0 errors，0 skipped。
- `clean test build`：通过。
...
- 该检查点构建 SHA-256：
  `CE50CE2899DAB3A2D259129BD51A659CA55124E96A12768519DA1B64D40D2850`。
...
- 通用双坐标完整游戏矩阵：阻挡、楼层、任务切换、女仆重载、多女仆竞争和长期
  Tick 型厨具仍需继续验证。
- 烹饪期移动约束：代码、48 项测试和 clean build 已通过；仍需在大半径河童罗盘下
  实测 FD 厨锅与蒸笼的等待期静止、活动目标被中断后的补路，以及三次失败后重新
  选择侧面。
