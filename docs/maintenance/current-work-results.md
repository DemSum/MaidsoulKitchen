...
- 通用烹饪搜索的 120–239 tick 随机冷却不再因 TLM 闲置游荡占用
  ...
  厨具后才用工作落脚点覆盖它。
- 通用厨具和蒸笼共用 TLM 1.5.3 的 `MaidPathFindingBFS` 搜索底座；删除蒸笼重复的
  BFS 包装类，配方/中枢准备也由“每候选设备一次”收敛为“每轮搜索一次”。
- 每名女仆、每种任务只保存至多 64 个本轮已服务坐标。搜索优先尚未服务的可操作
  设备，全部轮过后才重新选择旧设备；16 个并排酒桶不会再由首个持续可操作设备
  长期饿死其余设备。
- 通用厨具也使用短期工作锁，避免多名女仆同时选择同一设备；锁随工作记忆清理，
  不建立世界级设备缓存。
...
- 按 KC 1.4.1 的 `SteamerBlockEntity.MAX_LIT_LEVEL` 识别最多四层连续蒸笼的
  向上传热。
- 搜索和交互距离覆盖第四层；女仆只允许站在连续蒸笼柱底层的同层侧面，从该位置
  逐层操作真实蒸笼坐标，不再把楼上节点作为同一蒸笼柱的落脚点。第五层超出 KC
  原生热传播上限。
...
- 最近完整测试：47 tests，0 failures，0 errors，0 skipped。
- `clean test build`：通过。
- NeoForge 21.1.244 开发服务器：最新启动至 `Done (4.599s)`。
...
- 该检查点构建 SHA-256：
  `B1BB8FE3D89AF954CDA0A09EEB9299ABF9994908CD57B8BDF5B60B39C0608CC5`。
...
- 多层蒸笼：已改为底层同层落脚并保留逐层真实坐标交互，等待 2/3/4 层、楼上有
  可达地板、半/整蒸笼、断火恢复和逐层入库实测。
- 多设备轮转：代码与纯状态测试已完成；仍需用 16 个并排 DrinkBeer 酒桶和多口 FD
  厨锅确认逐个服务。实心 4×4 阵列中央四格没有可达侧面，本设计会有意跳过。
